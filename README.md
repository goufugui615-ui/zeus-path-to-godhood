...
项目内置完整统计方案：游戏埋点 → 云端存储（Cloudflare Worker + KV，免费）→ 本地仪表盘。

- 想看**多少人点进来**：独立访客 UV（按 IP 每日去重）、访问次数 PV
- 想看**繁衍了多少后代**：后代总数、今日新增、品质分布（凡人→传奇神裔）
- 还能看：开局次数、求爱次数、登神人数、被赫拉变成牛的次数、人均后代、近 30 天明细
...
![数据后台](docs/v2-stats-dashboard.png)

> 不想碰云服务？把 `mode` 改成 `'la'` 走 51.LA 自定义事件（零部署，去 51.LA 后台看）。
> 默认 `mode: 'off'`，Fork 的人不会把数据算到你头上。详见 [docs/数据统计说明.md](docs/数据统计说明.md)。
